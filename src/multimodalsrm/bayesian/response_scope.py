...
def validate_posterior_responses(responses, order):
    ...
    if any(f not in (Identity, Gaussian, Gamma, DoubleGamma, BachSCR) for f in families):
        raise ValueError(
            "posterior responses support Identity/Gaussian/Gamma/DoubleGamma/BachSCR; "
            "sampled responses remain unsupported"
        )
    if order is None and any(f in (Gamma, DoubleGamma, BachSCR) for f in families):
        raise ValueError(
            "Gamma/DoubleGamma/BachSCR posterior responses require explicit "
