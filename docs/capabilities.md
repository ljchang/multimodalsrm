...
| GP MAP, `linear_algebra="dense"` or `"grouped"` | Continuous Matérn GP; analytic Identity/Gaussian; explicit quadrature for Gamma/DoubleGamma/BachSCR/BatemanSCR | Grouped algebra preserves the modeled likelihood; it is not a different prior |
| GP MAP, `linear_algebra="state_space"` | Explicit opt-in Identity, Gaussian, fixed integer-shape Gamma/DoubleGamma, fixed-shape BachSCR, and learned BatemanSCR; independent noise | Response approximations and restored tails need the declared covariance error tolerance; no state-space posterior sampling |
