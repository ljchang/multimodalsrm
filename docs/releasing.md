...
Successful registration or CI does not prove an upload succeeded. Verify the published project version and clean installation after the first real deployment. A development version must be replaced by a reviewed release version before an upload can pass the version gate.
